...
const surveySlice = createSlice({
  ...
  reducers: {
    ...
    updateSurvey: (
      state,
      action: PayloadAction<{ id: string; updates: Partial<Omit<Survey, 'id' | 'createdAt'>> }>
    ) => {
    ...
    addQuestion: (
      state,
      action: PayloadAction<Omit<Question, 'id' | 'order'> & { order?: number }>
    ) => {
    ...
    updateQuestion: (
      ...
      action: PayloadAction<{
        id: string;
        surveyId: string;
        updates: Partial<Omit<Question, 'id' | 'surveyId' | 'order'>>;
      }>
    ) => {
    ...
    reorderQuestions: (
      state,
      action: PayloadAction<{ surveyId: string; questionIds: string[] }>
    ) => {
      const questions = state.questions[action.payload.surveyId];
      if (questions) {
...
// Selectors
export const selectAllSurveys = (state: any) => state.survey?.surveys || [];
export const selectSurveyById = (state: any, id: string) =>
  state.survey?.surveys?.find((s: Survey) => s.id === id);
export const selectQuestionsBySurveyId = (state: any, surveyId: string) =>
