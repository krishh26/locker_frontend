...
export const updateFormDataAPI = (data) => async (dispatch) => {
  try {
    ...
    const response = await axios.patch(
      `${URL_BASE_LINK}/form/update/${data.id}`,
      payload
    )
    dispatch(
      showMessage({ message: response.data.message, variant: 'success' })
...
export const getFormDataAPI =
  (data = { page: 1, page_size: 10 }, search_keyword = '', user_id = '') =>
  async (dispatch) => {
    try {
...
export const createUserFormDataAPI = (data) => async (dispatch) => {
  try {
    ...
    const response = await axios.post(
      `${URL_BASE_LINK}/form/user/create`,
      data,
      {
        headers: {
...
export const AddUsersToForm = (form_id, data) => async (dispatch) => {
  try {
    ...
    const { ...payload } = data
    const response = await axios.patch(
      `${URL_BASE_LINK}/form/add-user/${form_id}`,
      payload
    )
    dispatch(
      showMessage({ message: response.data.message, variant: 'success' })
...
export const getUserAllFormAPI =
  (data = { page: 1, page_size: 10 }, search_keyword = '') =>
  async (dispatch) => {
    try {
      dispatch(slice.setLoader())
...
export const createTemplateData = (data) => async (dispatch) => {
  try {
    const response = await axios.post(
      `${URL_BASE_LINK}/form-template/create`,
      data
    )
    if (!response.data.status) {
      dispatch(
...
export const updateTemplate = (id, data) => async (dispatch) => {
  try {
    const response = await axios.patch(
      `${URL_BASE_LINK}/form-template/update/${id}`,
      data
    )
    if (response.data.status) {
...
export const deleteTemplate = (id) => async (dispatch) => {
  try {
    const response = await axios.delete(
      `${URL_BASE_LINK}/form-template/delete/${id}`
    )
    if (response.data.status) {
      dispatch(fetchTemplateData())
    } else {
