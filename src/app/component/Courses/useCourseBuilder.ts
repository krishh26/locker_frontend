...
import { courseReducer, initialState, formatText, EMPTY_COURSE_DATA } from './courseReducer';
...
const getStepFromLocalStorage = (courseId: string | undefined): number | null => {
...
export const useCourseBuilder = (edit = 'create') => {
  ...
  const [courseSaved, setCourseSaved] = useState(edit === 'edit' || Boolean(courseId));
  ...
  const [activeStep, _setActiveStep] = useState(savedStep !== null ? savedStep : 0);

  // Wrapper for setActiveStep that also saves to localStorage
  const setActiveStep = (stepOrFunction: number | ((prevStep: number) => number)) => {
    _setActiveStep((prevStep) => {
      const newStep = typeof stepOrFunction === 'function'
  ...
  const mapUnitsToFormat = useCallback((units: any[]) => {
    ...
    if (units && units.length > 0) {
      units.forEach(unit => {
        ...
        const mandatoryValue = typeof unit.mandatory === 'boolean'
          ? (unit.mandatory ? 'true' : 'false')
          : (unit.mandatory === 'true' ? 'true' : 'false');
  ...
  useEffect(() => {
    ...
    if (preFillData.units && preFillData.units.length > 0) {
      ...
      const hasValidUnits = preFillData.units.every((unit: any) =>
        unit && typeof unit === 'object' && unit.id && unit.title
      );

      if (!hasValidUnits) {
        console.warn('useCourseBuilder - Some units are missing required fields:', preFillData.units);
      }
  ...
  useEffect(() => {
    ...
    if (edit === 'edit' || edit === 'view') {
      ...
      if (state.courseData.course_core_type === 'Gateway') {
        appropriateStep = 0;
      } else if (directEditMode && editCourseId === courseId && Object.values(state.mandatoryUnit).length > 0) {
        appropriateStep = 1;
      ...
      if (savedStep !== null) {
        ...
        // For other course types, ensure the saved step is valid based on course state
          ...
          if (savedStep === 2 && !Object.values(state.savedUnits).some(saved => saved)) {
  ...
  const courseHandler = (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const saveCourse = async () => {
    ...
    if (state.courseData.course_core_type === 'Gateway' && !courseDataToSend.level) {
    ...
    if (courseSaved && (courseId || preFillData?.course_id)) {
      ...
      response = await dispatch(updateCourseAPI(courseIdToUpdate, { ...courseDataToSend, units }));
      if (response) {
        setCourseSaved(true);
        if (edit === 'edit') {
      ...
      if (response && response.data && response.data.course_id) {
        ...
        if (activeStep === 0 && state.courseData.course_core_type !== 'Gateway') {
          handleNext();
        }
      }
    }
  ...
  const handleStepClick = (step: number) => {
    ...
    if (step === 2 && state.courseData.course_core_type !== 'Gateway') {
      ...
      const hasSavedUnits = Object.values(state.savedUnits).some(saved => saved);
