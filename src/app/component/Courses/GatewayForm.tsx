...
import { TextField, Typography, Autocomplete, Box, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, CircularProgress } from '@mui/material';
...
const GatewayForm: React.FC<GatewayFormProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchStandardCourses = async () => {
      ...
      try {
        ...
        const assignedIds = courseData.assigned_standards?.map(s => s.id.toString()) || [];

        const assigned = standardItems.filter(item => assignedIds.includes(item.id));
        const available = standardItems.filter(item => !assignedIds.includes(item.id));
  ...
  useEffect(() => {
    ...
    const currentAssignedIds = courseData.assigned_standards?.map(s => s.id) || [];
  ...
  const handleCoursesAssigned = (assigned: CourseItem[], remaining: CourseItem[]) => {
    setAssignedCourses(assigned);
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <Autocomplete
                      ...
                      onChange={(_, newValue) =>
                        updateChecklistItem(item.id, 'evidenceRequired', newValue === "Yes")
                      }
                      disabled={edit === "view"}
                      options={["Yes", "No"]}
                  ...
                  <TableCell>
                    <Autocomplete
                      ...
                      onChange={(_, newValue) =>
                        updateChecklistItem(item.id, 'isDropdown', newValue === "Yes")
                      }
                      disabled={edit === "view"}
                      options={["Yes", "No"]}
                      renderInput={(params) => <TextField {...params} />}
                  ...
                  <TableCell>
                    <TextField
                      ...
                      onChange={(e) => updateChecklistItem(item.id, 'dropdownOptions', e.target.value)}
