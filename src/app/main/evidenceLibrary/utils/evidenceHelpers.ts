...
export const getUnitMappingStatus = (
  evidence: { mappings?: Array<{ unit_code: string; sub_unit_id: number | null; learnerMap: boolean; trainerMap: boolean; signedOff?: boolean; course?: { course_core_type?: string } }> },
  ...
  if (evidence.mappings && Array.isArray(evidence.mappings)) {
    for (const mapping of evidence.mappings) {
      const isQualificationCourse = mapping.course?.course_core_type === 'Qualification'
      ...
      if (isSubUnit) {
        ...
        if (isQualificationCourse && mapping.sub_unit_id === null && String(mapping.unit_code) === String(unitId)) {
          return {
            learnerMap: mapping.learnerMap === true,
            trainerMap: mapping.trainerMap === true,
            signedOff: mapping.signedOff === true
        ...
        if (mapping.sub_unit_id === null && String(mapping.unit_code) === String(unitId)) {
